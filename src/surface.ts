...
export function createSurfaceElements(surface: MR_DeviceSurface) {
  ...
  return {
    channels: createElements(8, (index) => {
      ...
      // const upperScribbleStrip = surface.makeCustomValueVariable("scribbleStrip");
      // upperScribbleStrip.mOnTitleChange = (activeDevice, objectTitle, valueTitle) => {
      ...
      return {
        ...
        faderTouched: surface.makeCustomValueVariable("faderTouch"),
      };
    }),
    ...
    control: {
      ...
      buttons: {
        ...
        misc: createElements(21, (index) =>
          makeSquareButton(
            46 + (index % 7) * 2.625,
            17 + Math.floor(index / 7) * 2.5 + (index < 14 ? 0 : 0.5)
          )
        ),
        transport: createElements(5, (index) => surface.makeButton(46.25 + index * 3.56, 25, 3, 2)),
        navigation: {
          bank: {
